...
describe("Verify author counting", () => {
  ...
  beforeAll(() => {
    Author.countDocuments = jest.fn(); // empty mock function
  });
  ...
  test("should return the count of authors based on first name filter", async () => {
    ...
    (Author.countDocuments as jest.Mock).mockImplementationOnce(
      // mock this Mongoose method to avoid dealing with the real database
      (filter: FilterQuery<IAuthor>) =>
    ...
    // Above tests two aspects: 1. Does getAuthorCount() correctly pass filters to countDocuments()?
    //2. Does getAuthorCount() return the expected count based on countDocuments()?
  });
