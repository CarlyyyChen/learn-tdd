...
describe("Verify POST /create_book", () => {
  it("should create a new book for an existing author and genre", async () => {
    ...
    // prototype exists for every ts objects, mocking the method in prototype helps mock the method for all instances
  ...
  it("should return error message with status 200 for invalid inputs", async () => {
    ...
    const response = await request(app).post("/newbook").send(mockReqBody);
    // assuming the save method will not be called, and therefore did not mock the save method implementation here
    expect(response.statusCode).toBe(200);
