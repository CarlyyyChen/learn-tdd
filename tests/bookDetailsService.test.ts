...
import request from "supertest"; // set mock requests without having to run the server, it mocks the express server
...
describe("Verify GET /book_dtls/:id", () => {
  ...
  beforeAll(() => {
    // will be run before all it() tests run
    consoleSpy = jest.spyOn(console, "error").mockImplementation(() => {}); //create a spy to test how and whether it is called
    // and mock the implementation of the spy, since in the test we are only interested in if it is getting called
    // we mock the implementation to be empty
  });

  afterAll(() => {
    // will be run after all it() tests have been run
    consoleSpy.mockRestore(); // restore the implementation of the spy
  });

  it("should respond with book title, author name, and book instances with imprint and status", async () => {
    // it.only(): only run this test within the test file
    const mockBook = {
      title: "Test Book",
  ...
  it("should respond with 404 if book id is empty", async () => {
    ...
    // only want to test one failing condition ????
  ...
  it("should respond with 404 if book is not found", async () => {
    // the id is not empty but there is no book found for that id
  ...
  it("should respond with 500 if there is an error fetching the book", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalled(); // test if the error is logged in the console
  });
